...
import { weightedScore, passBar, withQuizBonus, quizAvgFor, quizBonus, SCORE_WEIGHT, isWeekend } from "../src/lib/labels.ts";
...
// ── วันหยุด: เสาร์-อาทิตย์ ไม่ถูกนับเป็น "ไม่ลงเวลา" ─────────────────────────
assert.equal(isWeekend("2026-07-25"), true, "เสาร์ต้องเป็นวันหยุด");
assert.equal(isWeekend("2026-07-26"), true, "อาทิตย์ต้องเป็นวันหยุด");
assert.equal(isWeekend("2026-07-24"), false, "ศุกร์ต้องเป็นวันทำงาน");
assert.equal(isWeekend("2026-07-27"), false, "จันทร์ต้องเป็นวันทำงาน");
// ต้องทนทั้งรูปแบบ YYYY-MM-DD และ ISO เต็ม (ค่าที่ไหลมาจาก DB)
assert.equal(isWeekend("2026-07-25T00:00:00.000Z"), true, "ISO เต็มก็ต้องอ่านวันถูก");

console.log("✅ ตรรกะคะแนน + วันหยุด ผ่านทั้งหมด");
